...
NDFSM<-function(d,X_1,X_2,burn_iter,mcmc_iter,thin,CCA_select)
{
  ...
  { 
      ...
      
    {
      ...
      if(iter > 0.5*burn_iter){
        
        GHS_Run_1<-GHS_modified_2(X=X_delta_1,Lambda_sq=Lambda_sq_GHS_1,Nu=Nu_GHS_1,Omega=Omega_1_GHS,Sigma=Phi_1,tau_sq =tau_sq_GHS_1,xi=xi_GHS_1)
        
        Phi_1_iter<-GHS_Run_1$Sigma # name changed from Phi_1 to Phi_1_iter to show that this is from current iteration
        
        Omega_1_GHS<-GHS_Run_1$Omega
        
        Lambda_sq_GHS_1<-GHS_Run_1$Lambda_sq; Nu_GHS_1<-GHS_Run_1$Nu; tau_sq_GHS_1<-GHS_Run_1$Tau_sq ;xi_GHS_1<-GHS_Run_1$xi
        
        GHS_Run_2<-GHS_modified_2(X=X_delta_2,Lambda_sq=Lambda_sq_GHS_2,Nu=Nu_GHS_2,Omega=Omega_2_GHS,Sigma=Phi_2,tau_sq =tau_sq_GHS_2,xi=xi_GHS_2)
        
        Phi_2_iter<-GHS_Run_2$Sigma # name changed from Phi_2 to Phi_2_iter to show that this is from current iteration
        
        Omega_2_GHS<-GHS_Run_2$Omega
        
        Lambda_sq_GHS_2<-GHS_Run_2$Lambda_sq; Nu_GHS_2<-GHS_Run_2$Nu; tau_sq_GHS_2<-GHS_Run_2$Tau_sq ;xi_GHS_2<-GHS_Run_2$xi
      ...
      for(k in 2:d){
        ...
        Epost_iter[k]<-rinvgamma(1,1,((1/alpha^2)+(1/eta_iter[k])))
        
        eta_iter[k]<-rinvgamma(1, 0.5*(p1+p2)*(d-k+1), 1/Epost_iter[k] +
      ...
      CCA_MCMC[,,iter]<-cc_cal$CCA
      
      Direction_CCA_Vec1[,,iter]<-cc_cal$Direction_CCA_Vec1
      
      Direction_CCA_Vec2[,,iter]<-cc_cal$Direction_CCA_Vec2
      ...
      {   
        ...
        #phi_post_1_thin[,,i]<-Phi_1_iter # Post Variance_covariance matrix of View 1
        
        #phi_post_2_thin[,,i]<-Phi_2_iter# Post Variance_covariance matrix of View 2
        ...
        Phi_grand_MCMC_thin<-sigma_grand_cal(A_1_thin[,,i],A_2_thin[,,i],solve(Omega_post_1_thin[,,i]),solve(Omega_post_2_thin[,,i]),p1,p2)
...
# Output<-NDFSM(d=20,burn_iter=burn_iter,mcmc_iter=mcmc_iter,no_patients=no_patients,X_1=X_1,X_2=X_2,CCA_select=CCA_select)
# 
# ########### Storage##############################################
# 
# filename_output<-paste0(20,"NDFSM_BC_Output",sep="")
# 
# save(NDFSM_Output,file=paste(filename_output,".Rdata",sep=""))
