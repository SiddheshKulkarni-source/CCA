...
GHS_modified_2<-function(X,
                         Lambda_sq,Nu,Omega,Sigma,tau_sq,xi,iter){
                 ...
                 mcmc_iter,thin,CCA_select)
{
  ...
  no_patients <- nrow(X_1)   # FIX: define inside
  ...
  return(GHS_output<-list(Omega_grand=Omega_grand_thin,CCA_MCMC=CCA_thin,Direction_CCA_Vec1_MCMC=Direction_CCA_Vec1_thin,Direction_CCA_Vec2_MCMC=Direction_CCA_Vec2_thin,log_det_MCMC=log_det_MCMC_thin,likelihood_MCMC=likelihood))
