...
HDI_function_1<-function(col_data,true_val)
{
  ...
  ci_hdi <- quantile(col_data,c(0.025, 0.975))   # Calculate Credible Interval
...
HDI_function_2<-function(col_data,true_val)
{
  col_data<-Re(col_data)
  
  true_val<-Re(true_val)
  
  ci_hdi <- quantile(col_data,c(0.10, 0.90))   # Calculate Credible Interval
...
HDI_function_3<-function(col_data,true_val)
{
  col_data<-Re(col_data)
  
  true_val<-Re(true_val)
  
  ci_hdi <- quantile(col_data,c(0.25, 0.75))   # Calculate Credible Interval
...
Analysis_Summary<-function(Output,CCA_select, shrinkage_threshold=0.2,plot_indicator, plot_components=10)
{
...
HDI_CCA_1<-array(dim=c(CCA_select,2,no_experiment))
HDI_CCA_2<-array(dim=c(CCA_select,2,no_experiment))
HDI_CCA_3<-array(dim=c(CCA_select,2,no_experiment))

HDI_Direction_vector_1_1<-array(dim=c(p1,2,no_experiment))
HDI_Direction_vector_1_2<-array(dim=c(p1,2,no_experiment)) 
HDI_Direction_vector_1_3<-array(dim=c(p1,2,no_experiment))


HDI_Direction_vector_2_1<-array(dim=c(p2,2,no_experiment)) 
HDI_Direction_vector_2_2<-array(dim=c(p2,2,no_experiment)) 
...
for(j in 1:CCA_select)
{
  ...
  Zero_Count_CCA_1[,j,ex]<-HDI_function_1(col_data=CCA_thin[,j,],true_val=0)$Count
  
  Zero_Count_CCA_2[,j,ex]<-HDI_function_2(col_data=CCA_thin[,j,],true_val=0)$Count
  
  HDI_cal<-HDI_function_3(col_data=CCA_thin[,j,],true_val=0)
...
for(i in 1:p1)
{
  ...
  for(j in 1:CCA_select)
  {
    ...
    Zero_Count_Direction_vec_1_1[i,j,ex]<-HDI_function_1(col_data=Direction_CCA_Vec1_thin[i,j,],true_val=0)$Count
    
    Zero_Count_Direction_vec_1_2[i,j,ex]<-HDI_function_2(col_data=Direction_CCA_Vec1_thin[i,j,],true_val=0)$Count
    
    HDI_Direction_Vector_1_3<-HDI_function_3(col_data=Direction_CCA_Vec1_thin[i,j,],true_val=0)
...
for(i in 1:p2)
{
  ...
  for(j in 1:CCA_select)
  {
    ...
    Zero_Count_Direction_vec_2_1[i,j,ex]<-HDI_function_1(col_data=Direction_CCA_Vec2_thin[i,j,],true_val=0)$Count
    
    Zero_Count_Direction_vec_2_2[i,j,ex]<-HDI_function_2(col_data=Direction_CCA_Vec2_thin[i,j,],true_val=0)$Count
    
    HDI_Direction_Vector_2_3<-HDI_function_3(col_data=Direction_CCA_Vec2_thin[i,j,],true_val=0)
...
#rownames(Zero_Count_Direction_vec_2_1)<-colnames(X_2_rna)

#rownames(Zero_Count_Direction_vec_2_2)<-colnames(X_2_rna)

#rownames(Zero_Count_Direction_vec_2_3)<-colnames(X_2_rna)
...
{
   plot(x=1:iter_saved,y=CCA_thin[,i,1:iter_saved],xlab='',ylab='', main=paste0("CC",i, sep=""),type="l")

  abline(h=CCA_MCMC_summary[i],col="red")

 }

 #mtext('Thinner Iteration', side = 1, outer = TRUE, line = 2)
# mtext('CC Value', side = 2, outer = TRUE, line = 2)
 ...
 {
   plot(x=1:iter_saved,y=Direction_CCA_Vec1_thin[i,1,1:iter_saved],xlab='',ylab='',main=paste0("Dir.Vec1",i, sep=""),type="l")
 ...
 for(i in 1:plot_components)
 {
   plot(x=1:iter_saved,y=Direction_CCA_Vec2_thin[i,1,1:iter_saved],xlab='',ylab='',main=paste0("Dir.Vec2",i, sep=""),type="l")
 ...
 plot(x=1:iter_saved, y=apply(Output$Log_likelihood_MCMC_Grand,2,sum), xlab="Iterations", ylab="Likelihood per iteration", main="Likelihood Plot", type="l")
 
 par(mfrow=c(1,1))
 
 plot(x=1:iter_saved, y=Output$log_det_MCMC[1,], xlab="Iterations", ylab="Log Determinant per iteration", main="Log Determinant Plot", type="l")
...
Summary_Analysis<-list(Shrinkage_calculator=Shrinkage_calculator, Direction_Vector_View_1_Summary=Direction_CCA_Vec1_GHS_summary, Direction_Vector_View_2_Summary=Direction_CCA_Vec2_GHS_summary, CC_bound=Zero_count_bounds,HDI_Direction_vector_1_3=HDI_Direction_vector_1_3,HDI_Direction_vector_2_3=HDI_Direction_vector_2_3,effecticve_size_first_CCA=effecticve_size_first_CCA,Zero_Count_Direction_vec_1_3=Zero_Count_Direction_vec_1_3, Zero_Count_Direction_vec_2_3=Zero_Count_Direction_vec_2_3, CCA_MCMC_summary=CCA_MCMC_summary)
